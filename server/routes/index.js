...
Model.Article.update({_id: _id}, {$push: {comment: newData}}, function(err, doc){
    if(err){
      console.log(err);
      return;
    }else{
...
//提交留言
router.post('/index_speak', function(req, res, next) {
  ...
  var user = req.body.user;
    ...
    Model.Messages.create(newData, function(doc){
      responseData.code = 200;
...
// 查看所有留言
router.get('/index_detail_message', function(req, res, next){
console.log(123)
  Model.Messages.find({}, function(err,doc){
      console.log(doc);
      responseData.code = 200;
      responseData.message = '查找所有留言成功';
      responseData.data = doc;
      res.json(responseData);
})
})
